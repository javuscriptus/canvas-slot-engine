// Локализация. Язык берётся из ?lang= или из настроек браузера.
// Добавление языка = добавление объекта в STRINGS, ничего больше.
...
export class I18n {
  ...
  t(key, ...args) {
    const v = STRINGS[this.lang][key] ?? STRINGS.ru[key] ?? key;
    return typeof v === "function" ? v(...args) : v;
  }

  symbol(key) {
    return SYMBOL_NAMES[this.lang]?.[key] || SYMBOL_NAMES.ru[key] || key;
  }
}
