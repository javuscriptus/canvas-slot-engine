...
function portrait(aspect, vw, vh) {
  const width = 1080;
  ...
  const margin = 18;
  const inset = 42;
  const cell = Math.floor((width - margin * 2 - inset * 2) / 5);
  const gridW = cell * 5;
  const gridH = cell * 3;
...
function landscape(aspect, vw, vh) {
  const height = 1080;
  ...
  const cell = Math.floor(clamp((panelY - topBarBottom - 40 - inset * 2) / 3, 96, 200));
  const gridW = cell * 5;
  const gridH = cell * 3;
  const gridX = Math.round((width - gridW) / 2);
 ...
 */
export function buildLayout(vw, vh) {
  const aspect = vw / Math.max(1, vh);
  const layout = aspect >= 1 ? landscape(aspect, vw, vh) : portrait(aspect, vw, vh);
  if (!fullscreenSupported()) delete layout.topButtons.full;
  return layout;
}

/** Запасные раскладки для инициализации до первого измерения экрана. */
export const LAYOUTS = {
  landscape: landscape(16 / 9, 1920, 1080),
  portrait: portrait(9 / 19.5, 1080, 2340)
};

/** Вычисляемые из раскладки размеры игрового поля. */
export function gridMetrics(layout, reels = 5, rows = 3) {
  const w = layout.cell * reels;
  ...
  return {
    ...
    width: w,
    height: h,
    centerX: layout.grid.x + w / 2,
    centerY: layout.grid.y + h / 2,
    frame: {
      x: layout.grid.x - inset,
      y: layout.grid.y - inset,
      width: w + inset * 2,
      height: h + inset * 2
    }
  };
...
/** Центр ячейки в координатах холста. */
export function cellCenter(metrics, reel, row) {
  return {
    x: metrics.x + metrics.cell * (reel + 0.5),
    y: metrics.y + metrics.cell * (row + 0.5)
  };
}
